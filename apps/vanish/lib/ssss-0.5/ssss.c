...
#define RAND_TYPE_RANDSOURCE 0
#define RAND_TYPE_SEED 1
...
mpz_t poly;
int cprng;
int opt_rand_type = RAND_TYPE_RANDSOURCE;
long int seed;
...
{
  if (opt_rand_type == RAND_TYPE_RANDSOURCE){
     cprng_init();
     // read the seed from the random source
     if (read(cprng, &seed, sizeof(long)) < 0){
       close(cprng);
...
void random_read(mpz_t x){
  char buf[MAXDEGREE / 8];
  unsigned int count;
  for(count = 0; count < degree / 8; count += 1){
    buf[count] = (char)random(); // generate randomly
  }
...
int main(int argc, char *argv[])
{
  ...
  opt_help = argc == 1;
  while((i = getopt(argc, argv, "vDhqQxs:t:n:w:r:")) != -1)
    switch(i) {
    ...
    case 'r': opt_rand_type = RAND_TYPE_SEED; 
    	      seed = atol(optarg);
              break;
    default:
      exit(1);
  ...
  if (strstr(name, "split")) {
    ...
    if (opt_help || opt_showversion) {
      puts("Combine shares using Shamir's Secret Sharing Scheme.\n"
	   "\n"
	   "ssss-combine -t threshold [-x] [-q] [-Q] [-D] [-v] [-r seed]");
      if (opt_showversion)
	puts("\nVersion: " VERSION);
      exit(0);
    }
