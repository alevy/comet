	...
	implements DHTControl, DHTTransportRequestHandler
{
		...
		DHTOperationListener	_listener )
	{
		...
		DHTTransportValue	value = database.store( new HashWrapper( encoded_key ), _value, _flags, _life_hours, _replication_control );
	...
	putEncodedKey(
		...
		boolean				original_mappings )
	{
		putEncodedKey(encoded_key, description, value, timeout, original_mappings,
		...
		byte[]		unencoded_key )
	{
		...
		DHTTransportValue res = database.get( new HashWrapper( encoded_key ),
				new HashWrapper(getTransport().getLocalContact().getID()), new byte[] {});
	...
	{
		getSupport( encoded_key, readerId, payload, description, flags, max_values, timeout, exhaustive, high_priority, new DHTOperationListenerDemuxer(get_listener) );
	}
			...
			final DHTOperationListener	get_listener )
	{
		...
		task_set[0] = getSupport( encoded_key, null, null, description, flags, max_values, timeout, exhaustive, high_priority, demuxer );
	}
									...
									if ( diversified_keys.length > 0 ){
										...
										for (int j=0;j<diversified_keys.length;j++){
											...
											if ( !result.isCancelled()){
												...
												result.add(
													getSupport( diversified_keys[j], readerId, payload, "Diversification of [" + this_description + "]", flags, rem,  timeout, exhaustive, high_priority, get_listener ));
											}
										}
									}
								}								
						...
						while ( !cancelled )
						{
							...
							try
							{
														...
														if ( obs_value != null && ! obs_recurse ){
															...
															if ( Arrays.equals( obs_value, value_bytes )){
																...
																f_closest.sendFindValue( 
																	...
																	_lookup_id, readerId, payload, 1, flags );
								...
								if (value_search)
								{
									...
									closest.sendFindValue(replyHandler, lookup_id, readerId, payload, rem, flags);
								} else
								{
		...
		byte				flags )
	{
		...
		HashWrapper readerW = null;
		if (readerId != null) {
			readerW = new HashWrapper(readerId);
		}
		DHTDBLookupResult	result	= database.get( originating_contact, new HashWrapper( key ),
				readerW, payload, max_values, flags, true );
	...
	{	
		...
		while( it.hasNext()){
			...
			DHTDBLookupResult	result = database.get( null, null, key, null, 0, (byte)0, false );
